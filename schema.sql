...
CREATE SCHEMA items AUTHORIZATION devel;


CREATE TABLE items.template (
  id       serial,
  name     text,
  description text,
  PRIMARY KEY (id)
...
CREATE TABLE items.template_belongs_to (
...
CREATE TABLE items.template_contains (
  --id                   serial,
  parent_template_id   int,
  child_template_id    int,
  description          text,
...
CREATE TABLE items.template_property (
  id                   serial,
  template_id          int,
  property_id          int,
  PRIMARY KEY (id),
...
CREATE TABLE items.item_property (
  template_property_id int,
  item_id int,
  property_value float,
  FOREIGN KEY (template_property_id)
    REFERENCES items.template_property(id),
  UNIQUE (template_property_id, item_id)
);
